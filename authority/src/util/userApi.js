...
const projectUrl = process.env.NODE_ENV === 'development' ? '/' : '/lakers'
...
function deleteAllCookie(){
    document.cookie.split(";").forEach(function(c) { 
        ...
        if(c.indexOf('_lakers') === -1 || c.indexOf('username_lakers') > -1){
            return 
        }
...
function refresh() {
    const  refresh_token = getCookie('refresh_token') ? getCookie('refresh_token') : ''
...
function setCookie(cname,cvalue,exdays)
{
    ...
    document.cookie = cname + '_lakers' + "=" + cvalue + "; " + expires + "; path=/";
}
function getCookie(cname)
{
    var name = cname + '_lakers' + "=";
...
function delCookie(name)
{
    ...
    if(cval!=null){
        document.cookie= name + '_lakers' + "="+cval+"; " + expires + "; path=/";
    }
}
export {
    logOut,
