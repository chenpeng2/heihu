...
const DEFAULT_PAGESIZE = 10 //这是antd分页
...
class MaterialTableDemo extends React.Component {
    ...
    componentDidMount() {
        this.setState({
            ...
            toFirstPage:this.props.toFirstPage
        })
    }
    componentWillReceiveProps(nextProps,nextState){
        if(nextProps.toFirstPage!==this.props.toFirstPage){
            this.setState({toFirstPage:nextProps.toFirstPage})
        }
    }
    //添加数据
    create(newData) {
    ...
    render() {
        const { createData, deleteData, hasPaging, title, updateData, antdPagination } = this.props
        let data = this.props.data
        const { copyData, toFirstPage } = this.state
        let tableData = data && data.list
        if (copyData.length) {
            tableData = copyData.concat(data.list)
        }
        const showAntdPagination = data.pageInfo && data.pageInfo.total > data.pageInfo.pageSize
        return (
            <div className="edit-table-content">
                <Paper>
                    <MaterialTable
                        ...
                        localization={{  //翻译
                            body: {
                                ...
                                emptyDataSourceMessage:<Empty image={Empty.PRESENTED_IMAGE_SIMPLE} />,
                            },
                            header: {
                                actions: '操作'
                            },
                        ...
                        editable={{
                            ...
                            onRowUpdate: updateData ? (newData, oldData) => new Promise((resolve, reject) => {
                            ...
                            onRowDelete: deleteData ? oldData =>
                                new Promise(resolve => {
                                    setTimeout(() => {
                                        this.props.deleteData(oldData.parameterId || oldData).then(res => {
                                            this.refreshTable()
                                            resolve()
                                        })
                                    }, 600)
                    ...
                    />
                     {!antdPagination && hasPaging && <TablePagination  // 服务端分页
                    ...
                    />}
                    {showAntdPagination && antdPagination && <Pagination className="angular-pagination" 
                        size={'small'} current={toFirstPage ? 1 : (this.state.page ? this.state.page : 1)} showQuickJumper pageSize={data.pageInfo ? data.pageInfo.pageSize:DEFAULT_PAGESIZE} total={data.pageInfo ? data.pageInfo.total : DEFAULT_COUNTS} onChange={(data)=>this.onChangePage(data)} 
                    />}
                </Paper>
            </div>
        )
