...
class EditableCell extends React.Component {
    ...
    initChildren = (children,initialValue)=> {
        ...
        if(inputtype==='select'){
            return <Select disabled={true} value={initialValue} style={{ width: 120 }} >
                {options}
            </Select>
        }
        if(['e','ue','g','ge','l','le'].indexOf(record && record.condition)>-1){
...
class EditableTable extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            onlyShowTable:props.onlyShowTable
        };
    }
    componentWillMount(){
        ...
        columns[columns.length-1]={
            ...
            dataIndex: 'operation',
            render: (text, record) => {
                const { editingKey, onlyShowTable } = this.state;
              ...
              <Popconfirm title="确认取消吗?" okText="确定" cancelText="取消" onConfirm={() => this.cancel(record.key)}>
                <a>取消</a>
                    ...
                    <a disabled={editingKey===0 || editingKey || onlyShowTable} style={(editingKey===0 || editingKey || onlyShowTable)?{opacity:0.5}:{}} onClick={() => this.edit(record.key)}>
                        编辑
                    </a>
                );
            },
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.onlyShowTable!==this.props.editingKey) {
            this.setState({ onlyShowTable: nextProps.onlyShowTable})
        }
