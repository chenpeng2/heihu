...
class FilterForm extends React.Component {
  ...
  render() {
    ...
    const shops = defaultItem.concat( shopList.map( (item) => {
      if (item.title) {
        return <Option key={ item.key }>{ item.title }</Option> }
      }
    ) );
    const types = defaultItem.concat( typeList.map( (item) => { return <Option key={ item.id }>{ item.name }</Option> } ) );
