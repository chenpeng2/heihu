...
class HeatMapChart extends React.Component {
    ...
    doPaintHotMap(nextProps){
        ...
        if(!paretoData.resultList){return}
    ...
    UNSAFE_componentWillReceiveProps(nextProps){
        if(nextProps.paretoData != this.props.paretoData){
            if(!nextProps.paretoData.resultList){
                //更新空的 显示加载转圈
                this.setState({data:[]})
                return
            }
    ...
    render() {
        ...
        if(yData.length<10){
            sliderScale = 75;
        }else if(yData.length<20){
            sliderScale = 40;
        }else if(yData.length<30){
            sliderScale = 35;
        }else if(yData.length<40){
            sliderScale = 30;
        }else{
            sliderScale = 20;
        }
        const myOptions = {
            tooltip: {
                ...
                formatter: function(params) {
                    return  yData[params.value[1]] + '<br/>' + xData[params.value[0]] + '<br/>' + '次数：' + params.data[2];
                }
            },
            animation: false,
            grid: {
            ...
            dataZoom: [//给x轴设置滚动条
                ...
                },
                {
                    ...
                    bottom: 26,//底部的距离
                    start: 0,
                    end: yDimension==1 ? 25 : sliderScale,
                    handleIcon: 'M10.7,11.9H9.3c-4.9,0.3-8.8,4.4-8.8,9.4c0,5,3.9,9.1,8.8,9.4h1.3c4.9-0.3,8.8-4.4,8.8-9.4C19.5,16.3,15.6,12.2,10.7,11.9z M13.3,24.4H6.7V23h6.6V24.4z M13.3,19.6H6.7v-1.4h6.6V19.6z',
                    handleSize: 20,
                    showDetail: false,
                },
