...
const { Option } = Select;
const { Search } = Input;
class HubResult extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            currentView: '',
    ...
    upDateView = (params)=> {
        ...
        const { creator, defectType, imageProcessDatetime, isDefault, isQuery, isStared, pageId, permission, scrapReason, viewId, viewName, xdimension, ydimension} = viewItem
        startTime = startTime?moment(startTime).format('YYYY-MM-DD'):'';
        endTime = endTime?moment(endTime).format('YYYY-MM-DD'):'';
        if((!startTime && endTime) || (startTime && !endTime)){
            if(!startTime){message.warning('起始时间必填');return}
    ...
    getDetailData = (params) => {
        ...
        const pams = this.filterUrl(`${barcodeNo}${wheelProcessDatetime}${wheelType}${pageNum}${pageSize}`)
        this.setState({
            isLoading: true,
        })
        return request({
            url: `/wheelQuery/wheelResult?${pams}`,
            method: 'GET'
        }).then(res => {
            if(res && res.code === 0){
                let detailData = {}
                detailData.total=res.data.total;
                detailData.wheelDetectionImageList=res.data.wheelDetectionResultList;
                this.setState({
                    detailData,
                    isLoading: false,
                })
    ...
    chooseView2 = (item,index)=> {
        ...
        this.setState({viewData,viewDataIndexId:viewData[index].viewId},()=>{
            ...
            //跳转的参数如下
            const viewPage = viewData[index];
            const {scrapReason, wheelProcessDatetime, wheelType, viewId, barcodeNo} = viewPage;
            ...
            this.setState({scrapReason, startTime, endTime, wheelType, barcodeNo},()=>{
                ...
                this.setState({viewShow:false})
                if(item.isQuery=='Y'){
                    viewId==100006?this.updateData():this.submitSearch('restore')
                }
            });
        })
    }
    ...
    renderFilters() {
        ...
        const popContent =<div>
            ...
            <div className="pop-footer">
                {viewId!=="100006"?<Button type="primary" size="small" onClick={this.upDateView}>保存</Button>:null}
                ...
                <div className="main-panel-light">
                    <MTable
                        columns={[
                            ...
                            { title: '轮毂处理时间', field: 'wheelProcessDatetime'},
                            { title: '流水号', field: 'wheelBatchNo'},
                            { title: '轮毂型号', field: 'wheelType' },
                            { title: '轮毂检测结果', field: 'wheelResult'},
                            { title: '轮毂照片数', field: 'numberOfComponents'},
                            { title: '光机号', field: 'xrayNo' },
                        ]}
                        rows={detailData}
                        className="big-table"
                        canAction={false}
...
const WrappedHubResult = Form.create({ name: 'register' })(HubResult);
