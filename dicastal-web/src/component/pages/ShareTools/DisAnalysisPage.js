...
const { Option } = Select;
const { Search } = Input;
class DisAnalysisPage extends React.Component {
    ...
    //新增视图
    addView = (params)=> {
        let { pageId, defectType, startTime, endTime, wheelType, xDimension, yDimension }=this.state
        const { userInfo }=this.props
        ...
        return request({
            ...
            data:{
                ...
                ydimension: yDimension ? yDimension : '',
            }
        }).then(res => {
            if(res && res.code==0){
                this.getView();
    ...
    /*更新视图*/
    upDateView = (params)=> {
        let { defectType, startTime, endTime, wheelType, xDimension, yDimension, viewItem }=this.state
        const {barcodeNo, creator, imageProcessDatetime, isDefault, isQuery, isStared, pageId, permission, scrapReason, viewId, viewName,} = viewItem
        startTime = startTime?moment(startTime).format('YYYY-MM-DD'):'';
        endTime = endTime?moment(endTime).format('YYYY-MM-DD'):'';
        if((!startTime && endTime) || (startTime && !endTime)){
            if(!startTime){message.warning('起始时间必填');return}
    ...
    getDetailData = (params) => {
        ...
        yDimension=yDimension?yDimension:'2';
    ...
    chooseView2 = (item,index)=> {
        ...
        this.setState({viewData,viewDataIndexId:viewData[index].viewId},()=>{
            ...
            //跳转的参数如下
            const viewPage = viewData[index];
            const {defectType, wheelProcessDatetime, wheelType, viewId, xdimension, ydimension} = viewPage;
            ...
            this.setState({defectType, startTime, endTime, wheelType, xDimension:xdimension, yDimension:ydimension},()=>{
                //根据是否需要立即更新
                this.setState({viewShow:false})
                if(item.isQuery=='Y'){
                    viewId==100003?this.updateData():this.submitSearch('restore')
                }
            });
        })
    }
    ...
    renderFilters() {
        ...
        const { isShowFilter, startTime, endTime, wheelTypeData, wheelDefectTypeData, isPingFilters, wheelType, defectType,
        ...
        const popContent =<div>
            ...
            <div className="pop-footer">
                {viewId!=="100003"?<Button type="primary" size="small" onClick={this.upDateView}>保存</Button>:null}
            ...
            <div className="subtitle-panel detail-data">
                ...
                <div className="filter-content" style={{display: isShowFilter ? "flex" : "none"}}>
                    ...
                    <div className="filter-item">
                        ...
                        onChange={this.onChangeDefectType.bind(this)}
                        filterOption={(input, option) =>
                        option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0
                        }
                        >
    ...
    render() {
        const { isPingFilters, timeFilterData, paretoData, headMapName,isParetoData,isTimeFilterData, xDimension, yDimension } = this.state;
            ...
            <div className="main-panel-light analysis">
                        ...
                        <div className="chart-title clearfix">
                            ...
                            <div className="flt-left">
                                ...
                                <Select
                                    ...
                                    style={{ width: 180, height:26, margin: '0 15px' }}
                                    optionFilterProp="children"
                                    value={yDimension?yArr[yDimension-1]:undefined}
                                    placeholder={yArr[1]}
                ...
                <div className="charts-continer" dir="ltr">
                    <div className="chart-item-continer ms-Grid-col ms-sm12 ms-md12 ms-lg12">
                        ...
                        <ScatterChart clearChange={this.clearChange} chooseTime={this.state} hotData={timeFilterData} isParetoData={isTimeFilterData} changeTime={this.changeTime} styleSheet={{ height: '300px'}}/>
                    </div>
                </div>
            </div>
        </div>
...
const WrappedDisAnalysisPage = Form.create({ name: 'register' })(DisAnalysisPage);
