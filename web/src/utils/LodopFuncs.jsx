...
import { Alert, FormattedMessage } from 'src/components';
import { changeChineseToLocaleWithoutIntl } from 'utils/locale/utils';

let CreatedOKLodop7766 = null;
let CLodopIsLocal = null;
  ...
  return class extends React.Component {
    ...
    componentDidMount() {
      ...
      if (this.needCLodop()) {
        ...
        const src3 = 'http://172.168.1.132:8000/CLodopfuncs.js?priority=2';
        ...
        <div>
          {showAlert ? (
            <Alert
              message={
                <FormattedMessage
                  defaultMessage={'请确认安装Web打印服务CLodop（若此前已安装过，可{link}），成功后请刷新本页面'}
                  values={{
                    link: (
                      <a href="CLodop.protocol:setup" target="_self">
                        {changeChineseToLocaleWithoutIntl('点这里直接再次启动')}
                      </a>
                    ),
                  }}
                />
          ...
          {/*getLodop的调用会造成这个组件的重新渲染。注意控制次数*/}
          <Comp {...this.props} getLodop={this.getLodop} />
        </div>
      );
    }
  };
