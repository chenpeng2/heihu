...
import { sparePartsImportList } from 'src/services/equipmentMaintenance/spareParts';
...
class SparePartsImportHistory extends Component {
  props: Props;
  state = {
    loading: false,
    dataSource: null,
  ...
  getColumns = () => {
    const columns = [{
      ...
      title: '导入详情',
      dataIndex: 'detail',
      render: (text, record) => <div>备件导入完成！成功数：{record.successAmount}，失败数：{record.failureAmount}。</div>,
    }, {
      title: '操作',
      dataIndex: 'operation',
      render: (text, record) => (
        <div key={`code-${record.id}`}>
          <Link
            ...
            onClick={() => {
              this.context.router.history.push(`/knowledgeManagement/spareParts/importLog/detail/${record.importId}`);
            }}
          >
            查看
          </Link>
  ...
  fetchData = async (params, query) => {
    ...
    const res = await sparePartsImportList(params);
    const { data } = res.data;
    this.setState({
      dataSource: data,
      loading: false,
...
SparePartsImportHistory.contextTypes = {
  router: PropTypes.object,
};

export default SparePartsImportHistory;
