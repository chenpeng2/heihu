...
class ProdTaskDetail extends Component {
  ...
  fetchData = () => {
    ...
    const fetchApi = baitingProdTask;
    fetchApi({ taskId })
      .then(res => {
        const { data } = res;
      ...
      <div>
        ...
        <ActionButton
          ...
          onClick={() => {
            ...
            this.context.router.history.push(`/cooperate/blankingTasks/detail/${taskId}/log`);
          }}
        >
          <Icon type="bars" />
  ...
  getInputMaterialTable = data => {
    ...
    const columns = [
      ...
      },
      {
        ...
        width: 400,
        key: 'amount',
        render: (amountInfo, record) => {
          const amount = _.get(record, ['0', 'amount'], replaceSign);
          const plannedAmount = _.get(record, ['0', 'plannedAmount'], replaceSign);
          return (
            <div>
    ...
    let dataSource = data.inputMaterials.map(v => [v, null]);
    if (Array.isArray(data.inputMaterialsPlanned)) {
      dataSource = data.inputMaterials.map((v, index) => [v, data.inputMaterialsPlanned[index]]);
    }
    return (
      <div
        style={{
  ...
  getOutputMaterialTable = data => {
    const { changeChineseToLocale } = this.context;
    let dataSource = data.outMaterials.map(v => [v, null]);
    if (Array.isArray(data.outMaterialPlanned)) {
      dataSource = data.outMaterials.map((v, index) => [v, [data.outMaterialPlanned][index]]);
    }
