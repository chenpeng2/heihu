...
import { getCheckTaskDetail, getCheckLogList } from 'src/services/equipmentMaintenance/checkTask';
...
class CheckTaskDetail extends Component {
  props: Props;
  state = {
    loading: false,
  ...
  fetchLogList = variables => {
    ...
    getCheckLogList(taskCode, variables)
      .then(res => {
  ...
  onDeleteTask = () => {
    ...
    DeleteTaskByStrategy(taskCode).then(() => {
      message.success('删除点检任务成功');
      history.push('/equipmentMaintenance/checkTask');
    });
  };
        ...
        <div>
          <div style={{ display: 'flex', margin: '25px 0 82px 0' }}>
            ...
            <div style={{ marginLeft: 10 }}>
              <h3>{changeChineseToLocale('删除点检任务', intl)}</h3>
          ...
          action={
            <div style={{ display: 'flex' }}>
              <Link
                ...
                onClick={() => {
                  history.push(`/equipmentMaintenance/checkTask/edit/${taskCode}`);
                }}
              >
                编辑
              </Link>
          ...
          <span>
            {logType === 'finish' ? (
              <Link
                onClick={() => {
                  history.push(
                    `/equipmentMaintenance/checkTask/detail/${record.taskCode}/report/${record.subTaskCode || '-'}`,
                  );
                }}
              >
                任务报告
...
CheckTaskDetail.contextTypes = {
  changeChineseToLocale: PropTypes.func,
  changeChineseTemplateToLocale: PropTypes.any,
};

export default withForm({}, injectIntl(CheckTaskDetail));
