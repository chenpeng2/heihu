...
import { getStorageLog } from 'src/services/knowledgeBase/storage';
...
class StorageOperateRecords extends Component<propTypes> {
  state = {
    data: null,
    loading: false,
  };
  ...
  fetchData = async params => {
    ...
    getStorageLog(code, variables).then(res => {
      const { data } = res || {};
  ...
  getTableColumns = () => {
    return [
      ...
      },
      {
        ...
        key: 'storageName',
        type: 'personName',
        render: storageName => <span>{storageName || replaceSign}</span>,
      },
      {
        title: '操作类型',
        dataIndex: 'type',
...
StorageOperateRecords.contextTypes = {
  router: PropTypes.object.isRequired,
};

export default withForm({}, StorageOperateRecords);
