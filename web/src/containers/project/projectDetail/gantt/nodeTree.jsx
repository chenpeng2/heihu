...
class NodeTree extends Component {
  props: { data: [], onNodeClick: () => {}, viewer: {}, relay: {}, disabled: boolean, fetchData: () => {} };
  state = {};

  // /* 一级工位被禁用二级工位也要禁用，后端没有支持，暂时用parent处理一下 */

  renderNode = node => {
    const { viewer, relay, disabled, onNodeClick, fetchData } = this.props;
    if (node.children) {
      return (
        <div>
          <Node
    ...
    return <Node viewer={viewer} fetchData={fetchData} relay={relay} style={{ padding: 10 }} node={node} disabled={disabled} />;
  };
