...
import { deepGrey, paleGreen } from 'src/styles/color/index';
...
const NodeRenderWithOnClick = (onClick, onMouseOver) => {
  class NodeRendererDefault extends Component {
    ...
    render() {
      ...
      let icon;
      if (node.material.category.value === 'raw') {
        icon = 'yuanliaoquan';
      } else if (node.material.category.value === 'semi') {
        icon = 'banchengpinquan';
      } else if (node.material.category.value === 'prod') {
        icon = 'chengpinquan';
      } else {
        throw new Error('未知的物料类型');
      }
      ...
      return (
        ...
        >
          {toggleChildrenVisibility && (
            <div>
              <div className={baseStyles.prefix} style={{ left: -0.5 * scaffoldBlockPxWidth }}>
                <Icon
                  className="icon"
                  style={{
                    color: deepGrey,
                  ...
                  type={icon}
                />
                <div style={{ marginTop: 5 }}>{node.prefix}</div>
              </div>
              {node.expanded && !isDragging && <div style={{ width: scaffoldBlockPxWidth }} className={styles.lineChildren} />}
            </div>
          )}
            ...
            {connectDragPreview(
              ...
              >
                ...
                <div
                  ...
                  style={{ backgroundColor: node.selected ? paleGreen : 'transparent' }}
