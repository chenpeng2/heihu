...
import { RestPagingTable, Link, Button, Badge, Tooltip, FormattedMessage } from 'src/components';
import { getQuery } from 'src/routes/getRouteParams';
import { queryMaterialImportLogs } from 'src/services/bom/material';
...
class MaterialImportHistory extends Component {
  ...
  getColumns = () => {
    const columns = [
      ...
      },
      {
        ...
        dataIndex: 'detail',
        render: (text, record) => {
          const { successAmount, failureAmount, updateAmount, updateFailedAmount } = record || {};
          return (
            <FormattedMessage
              defaultMessage={
                '物料导入完成！创建成功数：{successAmount}，创建失败数：{failureAmount}；更新成功数：{updateAmount}，更新失败数：{updateFailedAmount}'
              }
              values={{
                successAmount,
                failureAmount,
        ...
        render: (text, record) => (
          <div key={`code-${record.id}`}>
            <Link
              ...
              onClick={() => {
                this.context.router.history.push(`/bom/materials/logs/import/${record.importId}`);
              }}
            >
              查看
            </Link>
  ...
  fetchData = async (params, query) => {
    ...
    const res = await queryMaterialImportLogs(params);
    const { data } = res.data;
    this.setState({
      dataSource: data,
      loading: false,
