...
import { getProcessOperationHistory } from 'src/services/process';
...
class OperationHistory extends Component {
  ...
  componentWillMount() {
    ...
    this.setState({ code: id ? decodeURIComponent(id) : null });
  }
  ...
  getAndSetData = p => {
    ...
    getProcessOperationHistory(code, { ...p, size: 10 }).then(res => {
      const { data } = res || {};
  ...
  getValueByTime = params => {
    ...
    if (_time && _time.length === 2) {
      this.getAndSetData({ fromAt: _time[0], toAt: _time[1], ...params });
    } else {
