...
import { amountValidator, lengthValidate, dotValidator, checkTwoSidesTrim } from 'components/form';
...
class Form extends Component {
      ...
      form.validateFieldsAndScroll((err, value) => {
        ...
        if (res && Array.isArray(res.data)) {
          ...
          const filteredData = data.filter(i => {
            const validObj = i && i.keyName && i.keyLength;
            return validObj;
          });
  ...
  getColumns() {
    ...
    const renderName = (data, record, index) => {
      const initialValue = _.get(record, 'keyName', '');
      return (
        <FormItem>
          {getFieldDecorator(`data[${index}].keyName`, {
            rules: [
              {
                required: true,
                message: '字段名称必填',
    ...
    const renderLength = (data, record, index) => {
      const initialValue = _.get(record, 'keyLength', '');
      return (
        <FormItem>
          {getFieldDecorator(`data[${index}].keyLength`, {
            rules: [
              {
                required: true,
                message: '字段长度必填',
            ...
            onChange: v => {
              const { dataSource } = this.state;
              dataSource[index].keyLength = v;
            },
          })(<InputNumber style={{ width: 100 }} placeholder="请输入" />)}
        </FormItem>
      );
    };
    const renderType = (data, record, index) => {
      const keyType = _.get(record, 'keyType', null);
      const initialValue = keyType == null ? KeyTypes[0].key : keyType;
      const { id } = record;
      const disabled = !isNaN(id);
      return (
        <FormItem>
          {getFieldDecorator(`data[${index}].keyType`, {
            rules: [],
            initialValue,
            onChange: v => {
              const { dataSource } = this.state;
              dataSource[index].keyType = v;
            },
