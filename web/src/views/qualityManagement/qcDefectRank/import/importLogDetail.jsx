...
import {
  getQcDefectRankImportLogErrorList,
  getQcDefectRankImportLogDetail,
...
const ImportDetail = (props: Props) => {
  ...
  const fetchLogDetail = () => {
    setLoading(true);
    getQcDefectRankImportLogDetail({ importId })
      .then(res => {
  ...
  const fetchLogErrorList = params => {
    ...
    getQcDefectRankImportLogErrorList({ importId, ...params })
      .then(res => {
    ...
    return (
      <div style={{ marginBottom: 30 }}>
        ...
        <Row>
          ...
          <Col type={'content'} style={{ width: 920 }}>{`导入完成，不良等级导入成功数：${
            data.successAmount
          }，不良等级导入失败数：${data.failureAmount}`}</Col>
        </Row>
      </div>
    );
  };
