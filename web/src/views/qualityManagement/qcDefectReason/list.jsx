...
import { Link, Badge, Table, Spin, message, openModal, Button, Checkbox, selectAllExport, FormattedMessage } from 'src/components';
import { replaceSign } from 'src/constants';
import { getQuery } from 'src/routes/getRouteParams';
import {
  createQcDefectReason,
  disabledQcDefectReason,
  enabledQcDefectReason,
  updateQcDefectReason,
  getQcDefectReasonList,
} from 'src/services/knowledgeBase/qcModeling/qcDefectReason';
import log from 'src/utils/log';
import { primary } from 'src/styles/color';
import Create from './create';
import Edit from './edit';
import QcDefectReasonImportButton from './import/importButton';
import { QCDEFECT_REASON_STATUS, qcDefectReasonHeaderDesc, qcDefectReasonHeader } from './constants';
  ...
  const getColumns = () => {
    return [
      ...
      },
      {
        ...
        render: description => description || replaceSign,
      },
      {
        title: '状态',
        width: 140,
        ...
        render: status =>
          (typeof status === 'number' && (
            <Badge.MyBadge
              color={QCDEFECT_REASON_STATUS[status].color}
              text={`${QCDEFECT_REASON_STATUS[status].label}中`}
            />
          )) ||
          replaceSign,
      },
      {
        ...
        render: (data, record) => {
          ...
          const updateQcDefectRankStatus = status ? disabledQcDefectReason : enabledQcDefectReason;
          return (
                    ...
                    openModal({
                      ...
                      onOk: () => {
                        ...
                        validateFieldsAndScroll((err, values) => {
                          if (err) return null;
                          setUpdating(true);
                          updateQcDefectReason(id, values)
                            .then(() => {
                              message.success('编辑不良原因成功');
    ...
    selectAllExport(
      ...
      },
      {
        ...
        getExportData: async params => {
          const res = await getQcDefectReasonList({ ...getFormatSearchParams(queryMatch), ...params });
          let exportData;
          if (isAllChecked) {
          ...
          return [qcDefectReasonHeaderDesc, qcDefectReasonHeader, ...values];
        },
        fileName: `不良原因_${moment().format(formatTodayUnderline())}`,
      },
    );
  };
    ...
    openModal({
      ...
      onOk: () => {
        ...
        validateFieldsAndScroll((err, values) => {
          if (err) return null;
          setUpdating(true);
          createQcDefectReason(values)
            .then(() => {
              message.success('创建不良原因成功');
              refreshData(query);
            })
            .finally(() => {
              setUpdating(false);
  ...
  return (
    <div>
      <div style={{ margin: '20px 0 0 20px', display: 'flex' }}>
        ...
        {renderExport()}
        <QcDefectReasonImportButton history={history} />
      </div>
