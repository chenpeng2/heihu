...
import { queryQcTaskList, updateQcTaskStatus } from 'src/services/qualityManagement/qcTask';
...
import { formatQcTaskListFilterData, getDefect, getCheckTypeDisplay } from '../../qcTask/utils';
import { qcTaskStatusMap, qcTaskStatusColorMap, QCTASK_STATUS_FINISHED, REPEAT_QCTASK_TYPE } from '../../constants';
import { RejectRepeatQcAuditLink, PassRepeatQcAuditLink, QcTaskOperationLogLink } from '../../qcTask/base';
import { toCreateRepeatQcAuditDetail, toQcTaskOperationLog, toQcConfigDetail } from '../../navigation';
...
class CreateRepeatQcAuditList extends Component {
  props: Props;
  state = {
    loading: false,
    dataSource: [],
    ...
    return [
      ...
      },
      {
        ...
        render: material => {
          ...
          return (
            <Link.NewTagLink href={`/bom/materials/${encodeURIComponent(material.code)}/detail`}>
              {display}
            </Link.NewTagLink>
          );
        },
      },
      {
        ...
        render: qcConfig => {
          ...
          return name && id ? <Link.NewTagLink href={toQcConfigDetail(id)}>{name}</Link.NewTagLink> : replaceSign;
        },
      },
      {
      ...
      },
      {
        ...
        render: (checkType, record) => {
          ...
          return getCheckTypeDisplay(qcTaskClassification, checkType);
        },
      },
      {
      ...
      },
      {
        ...
        render: (task, record) => {
          ...
          return processCode ? (
            <Link.NewTagLink href={`/bom/newProcess/${encodeURIComponent(processCode)}/detail`}>{`${processCode ||
              replaceSign}/${processName || replaceSign}`}</Link.NewTagLink>
          ) : (
            replaceSign
          );
        },
      ...
      },
      {
        ...
        render: purchaseOrderCode => {
          ...
          return (
            <Link.NewTagLink
              to={`/cooperate/purchaseOrders/${encodeURIComponent(purchaseOrderCode)}/detail?code=${purchaseOrderCode}`}
            >
              {purchaseOrderCode}
            </Link.NewTagLink>
          );
      ...
      },
      {
        ...
        render: (projectCode, record) => {
          ...
          return (
            <Link.NewTagLink href={`/cooperate/projects/${encodeURIComponent(projectCode)}/detail`}>
              {projectCode}
            </Link.NewTagLink>
          );
        },
      },
      {
        ...
        width: 140,
        fixed: 'right',
        render: ({ code, repeatReqId, repeatReqStatus, qcConfig }) => {
          const qcConfigName = _.get(qcConfig, 'name', replaceSign);
          const params = {
            taskCode: code,
            repeatQcReqId: repeatReqId,
            repeatQcReqStatus: repeatReqStatus,
            qcConfigName,
          };
          ...
          return (
            <div>
              <LinkWithAuth
                ...
                onClick={() => this.context.router.history.push(toCreateRepeatQcAuditDetail({ code }))}
              >
                查看
              </LinkWithAuth>
              <PassRepeatQcAuditLink style={LINK_STYLE} params={params} refetchData={this.fetchData} />
              <RejectRepeatQcAuditLink style={LINK_STYLE} params={params} refetchData={this.fetchData} />
  ...
  onSearch = () => {
    ...
    form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        if (sensors) {
          sensors.track('web_quanlity_repeateCreateApplyList_search', {
  ...
  fetchData = async params => {
    ...
    await queryQcTaskList({
      ...formatQcTaskListFilterData({
        ...query,
        ...params,
        status: QCTASK_STATUS_FINISHED,
        repeatQcTaskReq: REPEAT_QCTASK_TYPE,
      }),
...
CreateRepeatQcAuditList.contextTypes = {
  router: PropTypes.object.isRequired,
  changeChineseToLocale: PropTypes.func,
};

export default withForm({}, withRouter(CreateRepeatQcAuditList));
