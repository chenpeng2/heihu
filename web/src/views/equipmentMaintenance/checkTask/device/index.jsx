...
import { getCheckTaskList } from 'src/services/equipmentMaintenance/checkTask';
...
const DeviceCheckTask = (props: Props) => {
  const { match } = props;
  ...
  const fetchAndSetData = async value => {
    ...
    try {
      ...
      const res = await getCheckTaskList({ ...params, searchTaskCategory: 'check' });
...
export default withRouter(DeviceCheckTask);
