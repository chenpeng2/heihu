...
import { getMaintenanceTaskDetailUrl } from '../utils';
...
const MaintenanceTaskList = (props: Props) => {
  ...
  const getColumns = () => {
    const columns = [
      ...
      },
      {
        ...
        width: 150,
        key: 'tile',
        render: title => <Tooltip text={title} length={130} />,
      },
      {
        title: '目标名称',
      ...
      },
      {
        ...
        dataIndex: 'defCode',
        key: 'defCode',
        render: (defCode, record) => {
          const { defName } = record;
          if (!(defCode && defName)) {
            return replaceSign;
          }
          return <Tooltip text={`${defCode}/${defName}`} width={160} />;
        },
      },
      {
        title: '任务状态',
      ...
      },
      {
        ...
        render: (_, record) => (
          <Link
            onClick={() => {
              history.push(getMaintenanceTaskDetailUrl(record.taskCode));
            }}
          >
            {'详情'}
          </Link>
...
export default withRouter(injectIntl(MaintenanceTaskList));
